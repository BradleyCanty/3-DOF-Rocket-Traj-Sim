# -*- coding: utf-8 -*-
"""
rocket_traj_sim_ex10.py

Description:
    Continuing from exercise 9 and assuming an Earth Centered Inertial (ECI) frame, apply the velocity of the rocket 
    relative to the Earth's rotating atmosphere. This requires adapting the calculation for flight path angle to use 
    both the inertial velocity and the relative velocity: blend them based on the phase of flight.
    ...
    Notes regarding the ECI and ECEF frames: 
    The ECI frame is a non-rotating, inertial frame that has it's x-y plane coinciding with the equatorial plane of the 
    Earth, with its x-axis pointing toward the so-called "vernal equinox". Equinoxes are a common point of confusion and 
    deserve explanation: an equinox occurs when the earth is at a position in its orbit such that a vector from the Earth
    toward the sun points to where the ecliptic (i.e. the plane in which the Earth revolves around the sun) intersects 
    the celestial equator (i.e. the equatorial plane). The equinox which occurs near the first day of spring (with respect 
    to the North hemisphere) is called the vernal equinox: the Sun lies in the direction of the vernal equinox around 
    March 21.
    
    The Earth Centered, Earth Fixed frame is a rotating, non-inertial frame whose x-y plane conincides with the equatorial
    plane of the Earth, with its x-axis pointing from the center of Earth toward the intersection of the equatorial plane
    and the prime meridian.
    
    The J2000 frame is a standard, Earth-centered inertial reference frame (ECI) used in astronomy and astrodynamics. 
    It defines a fixed, non-rotating coordinate system based on the exact orientation of the Earth’s equator and the vernal 
    equinox at January 1, 2000, at 12:00 Terrestrial Time. The J2000 standard ECI frame is necessary because the Earth
    precesses and nutates over long time scales, so the vernal equinox changes over time. By using the J2000 standard
    we can determine the orientation of the ECEF frame at any future point in time.
...
v0_rel = np.array([0,0,0]) #initial velocities relative to launch pad
omega_earth = np.array([0,0,OMEGA_EARTH])
v0 = v0_rel + np.cross(omega_earth,r0) #initial velocity vector, accounting for rotating atmosphere
...
plt.grid(True)
plt.axis('equal')
plt.savefig(img_dir + '/1_range_vs_alt.png', dpi = 600)
plt.show()
...
plt.title(f"Pitch program: FPA = {round(pitch_programs['target_fpa_at_bo_deg'],2)}{deg_symbol} (actual {round(fpa_at_bo_deg,2)}{deg_symbol}) at {t_burnout_text} = {round(t_burnout,2)} seconds", fontsize=12, color="gray")
plt.xlabel('Time [sec]')
...
#5) dynamic pressure vs time
plt.figure()
plt.plot(t[:len(dynamic_pressure)],dynamic_pressure,'r')
plt.title('Dynamic Pressure vs Time')
plt.grid(True)
plt.xlabel('time [sec]')
plt.ylabel(r'dynamic pressure [$N/m^2$]')
plt.savefig(img_dir + '/5_dynamic_pressure_vs_time.png', dpi = 600)
plt.show()
