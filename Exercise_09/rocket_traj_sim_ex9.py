# -*- coding: utf-8 -*-
"""
rocket_traj_sim_ex9.py

Description:
    Starting from the previous exercise, replace the initial offset from vertical 
    with a vertical lift-off followed by a pitch over (i.e. implement a "pitch 
    program"). Specifically, implement it such that the rocket begins to pitch 
    over at an altitude of 30 meters and such that the flight path angle at 
    burnout matches one supplied by the user.
...
# Using ax.text with transAxes. X=0.05 (inside left), Y=1.05 (just above the top edge)
ax1.text(1.05, 0, text_str, transform=ax1.transAxes, fontsize=10, bbox=props, va='bottom')

plt.savefig(img_dir + '/v2_fpa_vs_time.png', dpi = 600, bbox_inches="tight")
plt.show()
...
big_ax.set_title("V2 Rocket Trajectory Metrics (ECI Frame)", fontsize=16, fontweight="bold", y=1.5)
...
plt.legend()
plt.grid(True)
plt.savefig(img_dir + '/v2_mach_num_and_drag_coeff_vs_time.png', dpi = 600)
plt.show()
...
def plot_traj(i,img_name):
    ...
    ax1.set_aspect('equal')
    ax1.grid()
    ax1.set_title("V2 Rocket Trajectory Across Limb of Earth (ECI frame)", fontsize=12)
    ax1.set_xlabel('Horizontal dist. from launch site [km]')
    ax1.set_ylabel('Vertical dist. from\nlaunch site [km]')
...
iio.mimsave('Images/v2_traj_across_limb_of_earth_gif.gif', frames, format='GIF', 
