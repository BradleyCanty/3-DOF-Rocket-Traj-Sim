...
from geophysical_tools import G, m_earth, r_earth, OMEGA_EARTH,get_air_density,get_mach_number
...
def get_state_dot(t,dt,state,stages,get_metrics=False,payload_separates=False):
    ...
    #Get pitch program variables: the dictionary key is the stage count
    ...
    if required_eta_deg == None:
        ...
        R = np.array([[math.cos(eta_rad),-math.sin(eta_rad)],[math.sin(eta_rad),math.cos(eta_rad)]])
    ...
    if t >= t_start_abs and t < coast_duration_sec:
        ...
        m_dot = stage.m_dot
        F_thrust = stage.v_exhaust * m_dot
        m_total = m - m_dot*dt
    ...
    Cd = get_drag_coeff(r_norm,v_relative_norm)
    a_drag = - (1/2 * get_air_density(h) * v_relative_norm**2 * stage.A * Cd) / m_total * drag_dir_scaling
    ...
    if get_metrics:
        M = get_mach_number(v_relative_norm,h)
        dynamic_pressure = .5 * get_air_density(h) * v_relative_norm**2
...
def propagate_traj(state0,tspan,dt,stages,launch_azimuth_deg,payload_separates=False,method='rk4',get_metrics=False):
    ...
    if get_metrics == False:
        ...
        Cd0 = get_drag_coeff(r0_norm,v0_relative_norm)
        h0 = r0_norm - r_earth
        M0 = get_mach_number(v0_relative_norm,h0)
        fpa_rad0 = get_flight_path_angle_rad(r0,v0_relative_norm)
        fpa_deg0 = fpa_rad0 * 180/np.pi
        angle_from_vel_vec_to_thrust_vec_deg0 = 0
        dynamic_pressure0 = .5 * get_air_density(h0) * v0_relative_norm**2
