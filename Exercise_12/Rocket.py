...
from geophysical_tools import r_earth, OMEGA_EARTH
import numpy as np
...
class Rocket():
    ...
    def get_phase_time_index_dict(self,end_is_bo=False):
        ...
        for i,stage in enumerate(self.config['stages'],1):
            if i == 1:
                phase_t_idx_dict['vertical flight'] = [get_t_idx(stage.t_start_abs),get_t_idx(stage.t_start_abs+stage.t_steer_start_rel)]
                phase_t_idx_dict['pitch over'] = [get_t_idx(stage.t_start_abs+stage.t_steer_start_rel),get_t_idx(stage.t_start_abs+stage.t_steer_start_rel+stage.steer_duration_sec)]
                phase_t_idx_dict['gravity turn'] = [get_t_idx(stage.t_start_abs+stage.t_steer_start_rel+stage.steer_duration_sec),get_t_idx(stage.t_bo_abs)-1]
            elif i == 2:
                phase_t_idx_dict['MECO, stage sep.'] = [get_t_idx(stage.t_start_abs)-1,get_t_idx(stage.t_start_abs+stage.coast_duration_sec)]
                phase_t_idx_dict['2nd stage burn'] = [get_t_idx(stage.t_start_abs+stage.coast_duration_sec),get_t_idx(stage.t_bo_abs)-1]
            elif i == 3:
                phase_t_idx_dict['SECO, stage sep.'] = [get_t_idx(stage.t_start_abs)-1,get_t_idx(stage.t_start_abs+stage.coast_duration_sec)]
                phase_t_idx_dict['3rd stage burn'] = [get_t_idx(stage.t_start_abs+stage.coast_duration_sec),get_t_idx(stage.t_bo_abs)-1]
            elif i == 4:
                phase_t_idx_dict['TECO, stage sep.'] = [get_t_idx(stage.t_start_abs)-1,get_t_idx(stage.t_start_abs+stage.coast_duration_sec)]
                phase_t_idx_dict['3rd stage burn'] = [get_t_idx(stage.t_start_abs+stage.coast_duration_sec),get_t_idx(stage.t_bo_abs)-1]
            else:
                phase_t_idx_dict[f'S{i} ECO, stage sep.'] = [get_t_idx(stage.t_start_abs)-1,get_t_idx(stage.t_start_abs+stage.coast_duration_sec)]
                phase_t_idx_dict[f'{i}th stage burn'] = [get_t_idx(stage.t_start_abs+stage.coast_duration_sec),get_t_idx(stage.t_bo_abs)-1]
            if i == len(self.config['stages']):
                if i == 1:
                    if self.config['payload_separates'] == True:
                        phase_t_idx_dict['MECO, stage sep.'] = [get_t_idx(stage.t_bo_abs)-1,get_t_idx(stage.t_bo_abs)]
                        phase_t_idx_dict['coast'] = [get_t_idx(stage.t_bo_abs),len(self.times)-1]
                    else:
                        phase_t_idx_dict['MECO, coast'] = [get_t_idx(stage.t_bo_abs),len(self.times)-1]
                elif i == 2:
                    if self.config['payload_separates'] == True:
                        phase_t_idx_dict['SECO, stage sep.'] = [get_t_idx(stage.t_bo_abs)-1,get_t_idx(stage.t_bo_abs)]
                        phase_t_idx_dict['coast'] = [get_t_idx(stage.t_bo_abs),len(self.times)-1]
                    else:
                        phase_t_idx_dict['SECO, coast'] = [get_t_idx(stage.t_bo_abs),len(self.times)-1]
                elif i == 3:
                    if self.config['payload_separates'] == True:
                        phase_t_idx_dict['TECO, stage sep.'] = [get_t_idx(stage.t_bo_abs)-1,get_t_idx(stage.t_bo_abs)]
                        phase_t_idx_dict['coast'] = [get_t_idx(stage.t_bo_abs),len(self.times)-1]
                    else:
                        phase_t_idx_dict['TECO, coast'] = [get_t_idx(stage.t_bo_abs),len(self.times)-1]
                elif i == 4:
                    if self.config['payload_separates'] == True:
                        phase_t_idx_dict['FECO, stage sep.'] = [get_t_idx(stage.t_bo_abs)-1,get_t_idx(stage.t_bo_abs)]
                        phase_t_idx_dict['coast'] = [get_t_idx(stage.t_bo_abs),len(self.times)-1]
                    else:
                        phase_t_idx_dict['FECO, coast'] = [get_t_idx(stage.t_bo_abs),len(self.times)-1]
                else:
                    if self.config['payload_separates'] == True:
                        phase_t_idx_dict[f'S{i} ECO, stage sep.'] = [get_t_idx(stage.t_bo_abs)-1,get_t_idx(stage.t_bo_abs)]
                        phase_t_idx_dict['coast'] = [get_t_idx(stage.t_bo_abs),len(self.times)-1]
                    else:
                        phase_t_idx_dict[f'S{i} ECO, coast'] = [get_t_idx(stage.t_bo_abs),len(self.times)-1]
