...
def get_state_dot(t,dt,state,stages,payload_separates=False):
    '''
    ...
    v_relative = v_inertial - np.cross(omega_earth,r) 
    ...
    if required_eta_deg == None:
        ...
        R = np.array([[np.cos(eta_rad),-np.sin(eta_rad),0],[np.sin(eta_rad),np.cos(eta_rad),0],[0,0,1]])
    ...
    state_dot = np.array([v_inertial[0],v_inertial[1], v_inertial[2], a[0], a[1], a[2], -m_dot])
    M = gt.get_mach_number(v_relative_norm,h)
    dyn_pres = .5 * gt.get_air_density(h) * v_relative_norm**2
    metrics = np.array([a_grav[0],a_grav[1],a_grav[2],a_thrust[0],a_thrust[1],a_thrust[2],a_drag[0],a_drag[1],a_drag[2],Cd,M,fpa_deg,eta_deg,dyn_pres])
    return state_dot,metrics

def propagate_traj(state0,tspan,dt,stages,launch_azimuth_deg,payload_separates=False,method='rk4'):
    ode         = get_state_dot
    ...
    r0 = np.array([rx0,ry0,rz0])
    r0_norm = np.linalg.norm(r0)
    
    v0_inertial = np.array([vx0,vy0,vz0])
    omega_earth = np.array([0,0,gt.OMEGA_EARTH]) 
    v0_relative = v0_inertial - np.cross(omega_earth,r0)
    v0_relative_norm = np.linalg.norm(v0_relative)
    
    a_grav0 = -gt.G*gt.m_earth/r0_norm**2 * r0/r0_norm
    a_thrust0 = stages[0].v_exhaust * stages[0].m_dot / m0 * r0/r0_norm
    a_drag0 = np.array([0,0,0])
    ...
    metrics[0] = [a_grav0[0],
                  ...
                  M0,
                  fpa_deg0,
                  angle_from_vel_vec_to_thrust_vec_deg0,
                  dynamic_pressure0]
    ...
    for step in range(steps - 1):
        ...
        states[step + 1], metrics[step + 1] = func(ode,time,dt,state,stages,payload_separates)
        
        #Check for impact
        rx_ = states[step+1,0]
        ry_ = states[step+1,1]
        rz_ = states[step+1,2]
        r_ = np.sqrt(rx_**2 + ry_**2 + rz_**2)
    ...
    if has_impacted: 
        #Truncate the states and metrics to the step at impact
        rx = states[:,0][:step_at_impact]
        ry = states[:,1][:step_at_impact]
        rz = states[:,2][:step_at_impact]
        vx = states[:,3][:step_at_impact]
        vy = states[:,4][:step_at_impact]
        vz = states[:,5][:step_at_impact]
        m  = states[:,6][:step_at_impact]
        
        ax_grav   = metrics[:,0][:step_at_impact]
        ay_grav   = metrics[:,1][:step_at_impact]
        az_grav   = metrics[:,2][:step_at_impact]
        ax_thrust = metrics[:,3][:step_at_impact]
        ay_thrust = metrics[:,4][:step_at_impact]
        az_thrust = metrics[:,5][:step_at_impact]
        ax_drag   = metrics[:,6][:step_at_impact]
        ay_drag   = metrics[:,7][:step_at_impact]
        az_drag   = metrics[:,8][:step_at_impact]
        Cd       = metrics[:,9][:step_at_impact]
        M        = metrics[:,10][:step_at_impact]
        fpa_deg  = metrics[:,11][:step_at_impact]
        eta_deg  = metrics[:,12][:step_at_impact]
        dyn_pres = metrics[:,13][:step_at_impact]
    else:
        rx = states[:,0]
        ry = states[:,1]
        rz = states[:,2]
    ...
    print('DONE\n')
    return [rx, ry, rz, vx, vy, vz, m], [ax_grav,ay_grav,az_grav,ax_thrust,ay_thrust,az_thrust,ax_drag,ay_drag,az_drag,Cd,M,fpa_deg,eta_deg,dyn_pres]
        ...
        if stage.required_eta_deg == None:
            ...
            while iter_ < max_iter:
                for j,eta_guess in enumerate(eta_guesses):
                    ...
                    states      = np.zeros((steps, len(state0)))
                    states[0]   = state0 #rx0,ry0,rz0,vx0,vy0,vz0,m0
...
def get_blended_vel_vec(h, v_rel, v_inertial, h_min=40000.0, h_max=80000.0):
    ...
    This is used for computing the steering velocity vector.
