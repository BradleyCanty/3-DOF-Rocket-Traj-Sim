...
Description:
    Continuing from exercise 7, extend the simulation to two dimensions. 
    Additionally, apply an initial offset from vertical to initiate a gravity turn.
...
plt.title(f'V2 Rocket Trajectory (ECI frame)\nMax range = {round(max(range_km),2)} [km], Max altitude = {round(max(altitude_km),2)} [km]')
plt.xlabel('Downrange [km]')
plt.ylabel('Altitude [km]')
plt.grid(True)
plt.axis('equal')
plt.savefig(img_dir + '/v2_downrange_vs_alt.png', dpi = 600)
plt.show()
...
ax1.grid()
ax1.set_title(f"V2 Rocket Trajectory Across Limb of Earth (ECI frame)\nPitch program: FPA = {round(fpa_at_bo_deg,2)}{deg_symbol} at {t_burnout_text} = {round(t_burnout,2)}[s], {eta_symbol} = {pitch_programs['eta_deg']}{deg_symbol}\nMax range = {round(max(range_km),2)} [km], Max altitude = {round(max(altitude_km),2)} [km]", fontsize=12)
ax1.set_xlabel('Horizontal dist. from launch site [km]')
ax1.set_ylabel('Vertical dist. from\nlaunch site [km]')
text_str = f"Pitch Program:\nStart time = {pitch_programs['t_pitch_prog_start']}[s]\nEnd time = {round(pitch_programs['t_pitch_prog_end'],2)}[s]\nTarget burnout FPA = {pitch_programs['target_fpa_at_bo_deg']}{deg_symbol}\nActual burnout FPA = {round(pitch_programs['fpa_at_bo_deg'],2)}{deg_symbol}\nSteering angle = {round(pitch_programs['required_eta_deg'],2)}{deg_symbol}\nSteering duration = {pitch_programs['steering_duration_sec']}[s]"
...
ax1.text(1.08, 0, text_str, transform=ax1.transAxes, fontsize=7, bbox=props, va='bottom')

plt.tight_layout()
plt.savefig(img_dir + '/v2_traj_across_limb_of_earth.png', dpi = 600)
plt.show()
...
plt.title(f"Pitch program: FPA = {round(fpa_at_bo_deg,2)}{deg_symbol} at {t_burnout_text} = {round(t_burnout,2)}[s], {eta_symbol} = {pitch_programs['eta_deg']}{deg_symbol}", fontsize=12, color="gray")
plt.xlabel('Time [s]')
plt.ylabel(f'Angle [{deg_symbol}]')
plt.grid(True)
plt.legend()
#plt.xlim(0,t_burnout+4)
plt.savefig(img_dir + '/v2_fpa_vs_time.png', dpi = 600)
plt.show()
...
big_ax.grid(False)
t_burnout_text = r'$t_{burnout}$'
big_ax.set_title(f"Pitch program: FPA = {round(fpa_at_bo_deg,2)}{deg_symbol} at {t_burnout_text} = {round(t_burnout,2)}[s], {eta_symbol} = {pitch_programs['eta_deg']}{deg_symbol}", fontsize=12, color="gray", pad=30, y=1.58)
...
ax3.grid()
fig.savefig(img_dir + '/v2_pos_vel_accel_vs_time.png', dpi = 600, bbox_inches="tight")
...
plt.legend()
plt.grid(True)
plt.savefig(img_dir + '/v2_mach_cd_vs_time.png', dpi = 600)
plt.show()
...
def plot_traj(i,img_name):
    ...
    ax1.set_aspect('equal')
    ax1.grid()
    ax1.set_title(f"V2 Rocket Trajectory Across Limb of Earth (ECI frame)\nPitch program: FPA = {round(fpa_at_bo_deg,2)}{deg_symbol} at {t_burnout_text} = {round(t_burnout,2)}[s], {eta_symbol} = {pitch_programs['eta_deg']}{deg_symbol}", fontsize=12)
    ax1.set_xlabel('Horizontal dist. from launch site [km]')
    ax1.set_ylabel('Vertical dist. from\nlaunch site [km]')
...
iio.mimsave('Images/v2_traj_gif.gif', frames, format='GIF', 
