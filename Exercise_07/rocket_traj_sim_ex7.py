...
from atmosphere_fxns import get_air_density,get_speed_of_sound
...
for i in range (len(t)-1):
    ...
    Cd[i] = get_Cd(M[i])
    rho_air[i] = get_air_density(h[i])
    if (t[i] < t_burnout):
        a_thrust[i+1] = v_exhaust * m_dot / (m_empty + m_propellant - m_dot*dt*(i+1))
        a_drag[i+1] = - (1/2 * rho_air[i] * vy[i] * abs(vy[i]) * A * Cd[i]) / (m_empty + m_propellant - m_dot*dt*(i+1))
        
    else:
        a_thrust[i+1] = 0
        a_drag[i+1] = - (1/2 * rho_air[i] * vy[i] * abs(vy[i]) * A * Cd[i]) / m_empty
